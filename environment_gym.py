# -*- coding: utf-8 -*-
"""
Created on Thu Aug 18 08:53:21 2022
...
class Env(gym.Env):
    ...
    def __init__(self,pa,nw_len_seqs=None, nw_size_seqs=None,
        ...
        self.observation_space = spaces.Box(low=0,high=1,shape=(int(pa.network_input_height)*int(pa.network_input_width),),
                                            dtype=np.float32)
    ...
    def observe(self):
        if self.repre == 'image':
            ...
            image_repr = np.zeros((int(self.pa.network_input_height), int(self.pa.network_input_width)),dtype=np.float32)
            ...
            for i in range(self.pa.num_res):
                ...
                ir_pt += self.pa.res_slot # serving job

                for j in range(self.pa.num_nw): # job in slots
    ...
    def step(self, a, repeat=False):
        ...
        return ob.reshape(-1,), reward, done, info
...
class Machine:
    def __init__(self, pa):
        ...
        self.colormap = np.arange(1 , pa.job_num_cap)
